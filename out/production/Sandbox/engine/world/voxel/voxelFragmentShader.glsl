...
void main(void)
{
   /* vec3 lightColour = vec3(1, 1, 1);
    ...
    float bright = dot(vec3(0, 1, 0), normalize(surfaceNormal));
    bright = mix(bright, bright * 0.1, step(0.0, -bright));
    bright = (bright + 1f) / 2;
    out_Color = vec4(colourPass, 1.0);// * (bright * 1f);
    vec4 surfaceColour = vec4((surfaceNormal + 0.3) / 2 + 0.5f, 1.0);
    ...
    out_Color = texture(texture2d, textureCoords * 1);
}